...
| `stateful` | seaweedfs | PVCs carry `argocd.argoproj.io/sync-options: Prune=false` from the first commit; `strategy: Recreate` for RWO volumes; hooks (the provisioning Job) run PostSync and are recreated each sync |
