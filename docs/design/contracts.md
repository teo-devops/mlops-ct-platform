...
|---|---|---|---|
| **artifact-store** | S3 API at `S3_ENDPOINT_URL`; **per use case** the buckets `<uc>-datasets`, `<uc>-models`, `<uc>-predictions` and the users `<uc>-reader` / `<uc>-pipeline` / `<uc>-api` whose policies name only them (+ the shared `mlflow` bucket); layout `<uc>/<model>/…` inside | MinIO (`workloads/minio`) | steps (`CT_*_BUCKET`), MLflow, KServe storage-initializer, API prediction log |
