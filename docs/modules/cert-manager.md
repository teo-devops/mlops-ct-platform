...
Wave 1. `startupapicheck` is off (a Helm-hook Job that flaps under Argo CD). KServe's `serving-cert` is issued by its own self-signed Issuer; the CA bundles injected into webhooks are ignored in Argo CD diffs (`config/argo-cd/values.yaml`).
