...
| **Disable** | `rollout.enabled: false` (back to a Deployment) and comment the line; then clean the selector the controller left on the stable Service — nobody else owns that key and the Application ignores it: `kubectl -n <uc>-api patch svc <uc>-api --type json -p '[{"op":"remove","path":"/spec/selector/rollouts-pod-template-hash"}]'` — otherwise the Service selects no pod (503) |
