...
SAMPLE = {"distance_km": 4.2, "weight_kg": 1.5, "hour": 18, "zone": "north"}


async def test_happy_path(client, models):
    r = await client.post("/v1/deliveries/eta", json=SAMPLE)
    assert r.status_code == 200, r.text
    body = r.json()
    assert set(body["predictions"]) == set(MODELS)
    assert all(p["model_version"] == "1" for p in body["predictions"].values())
    assert body["lineage"]["use_case"] == "delivery-eta" and body["lineage"]["request_id"]
...
async def test_schema_validation(client, models):
    r = await client.post("/v1/deliveries/eta", json={"distance_km": "far"})
    assert r.status_code == 422


async def test_metrics_exposed(client, models):
    await client.post("/v1/deliveries/eta", json=SAMPLE)
