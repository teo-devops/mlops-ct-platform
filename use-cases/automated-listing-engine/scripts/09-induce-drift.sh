...
#
#   REQUESTS=600 ./use-cases/automated-listing-engine/scripts/09-induce-drift.sh
...
docker run --rm "automated-listing-engine-steps:${VERSION:-0.1.0}" python -c \
  "from automated_listing_engine import data; print(data.generate(seed=7, rows=${REQUESTS}, profile='drift')[['title','price']].to_json(orient='records'))" \
...
for _ in $(seq 1 40); do
  kubectl -n argocd annotate application automated-listing-engine-serving automated-listing-engine-api argocd.argoproj.io/refresh=normal --overwrite >/dev/null 2>&1
