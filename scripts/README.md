...
| 06 | `use-case/06-build-images.sh` | `build` | steps + API images → `kind load` |
| 07 | `use-case/07-run-pipeline.sh` | `pipeline` | submit `ct-pipeline` for every model and wait (bootstraps the models on the first run) |
| 08 | `use-case/08-smoke.sh` | `smoke` | real request, fallback drill, network-policy probe |
| 09 | `use-case/09-induce-drift.sh` | `drift` | the closed loop: drifted world → monitor → retrain → promotion commit → new version served |
...
| — | `repo/new-use-case.py` | `new-use-case` | scaffold + register a use case from `repo/use-case-template/`: `NAME=x MODELS=a,b [TASK=regression] [API=false]` |
