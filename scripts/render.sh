...
kubectl kustomize "${REPO_ROOT}/gitops/bootstrap" > "${OUT}/00-bootstrap.yaml"
CHART_VERSION="$(awk '/chart: argo-cd/ {f=1} f && /targetRevision:/ {print $2; exit}' "${REPO_ROOT}/gitops/environments/demo/argo-cd.yaml")"
...
helm template argo-cd argo/argo-cd --version "${CHART_VERSION}" --namespace argocd --include-crds \
  --values "${REPO_ROOT}/gitops/argo-cd/values.yaml" > "${OUT}/02-engine-argocd.yaml"
