...
# workloads/seaweedfs/values.yaml) are minted here; each use case then copies the
...
SEAWEEDFS_ROOT_PASSWORD="${SEAWEEDFS_ROOT_PASSWORD:-seaweedfs-demo}"
...
for ns in "${ARTIFACT_STORE_NS}" mlflow observability; do
  kubectl create namespace "$ns" --dry-run=client -o yaml | kubectl apply -f - >/dev/null
done
info "${ARTIFACT_STORE_NS} mlflow observability"

step "Artifact store: root credentials (namespace ${ARTIFACT_STORE_NS})"
if exists "${ARTIFACT_STORE_NS}" seaweedfs-root; then info "kept"; else
  secret_upsert "${ARTIFACT_STORE_NS}" seaweedfs-root SEAWEEDFS_ROOT_USER=root "SEAWEEDFS_ROOT_PASSWORD=${SEAWEEDFS_ROOT_PASSWORD}"; info "created (root / \$SEAWEEDFS_ROOT_PASSWORD)"
fi

step "Artifact store: one secret key per consumer user (workloads/seaweedfs/values.yaml)"
...
import sys, yaml
for u in yaml.safe_load(open(sys.argv[1]))["users"]: print(u["name"], u["envPrefix"])' "${ARTIFACT_STORE_VALUES}")
secret_upsert "${ARTIFACT_STORE_NS}" "${ARTIFACT_STORE_USERS_SECRET}" "${users_kv[@]}"

step "Registry: artifact-store credential (namespace mlflow, user mlflow)"
store_user_secret mlflow mlflow-s3-credentials mlflow MLFLOW
